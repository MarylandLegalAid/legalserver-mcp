...
test('in-process MCP server lists canonical tools and serves representative calls', async () => {
  ...
  const fetchImpl = async (url) => {
    ...
    if (parsed.pathname === '/api/v1/contacts') {
      return jsonResponse(200, {
        data: [{
          id: 303,
          uuid: 'contact-uuid-1',
    ...
    if (parsed.pathname === '/api/v1/organizations') {
      return jsonResponse(200, {
        data: [{
          id: 505,
          uuid: 'org-uuid-1',
  ...
  const tasks = parseToolResult(await client.callTool({
    name: 'task_search',
    arguments: { title: 'Prepare filing' },
  }));
  ...
  const contacts = parseToolResult(await client.callTool({
    name: 'contact_search',
    arguments: { email: 'taylor@example.org' },
  }));
  assert.equal(contacts.data[0].contact_uuid, 'contact-uuid-1');

  const users = parseToolResult(await client.callTool({
    name: 'user_search',
    arguments: { login: 'jstaff' },
  }));
  assert.equal(users.data[0].user_uuid, 'user-uuid-1');

  const organizations = parseToolResult(await client.callTool({
    name: 'organization_search',
    arguments: { name: 'Legal Aid Partners' },
  }));
  assert.equal(organizations.data[0].organization_uuid, 'org-uuid-1');

  const userLookup = parseToolResult(await client.callTool({
    name: 'user_lookup_by_login',
    arguments: { login: 'jstaff' },
  }));
  assert.equal(userLookup.data.user_uuid, 'user-uuid-1');
