...
test('document records map text_strategy from mime type or extension', () => {
  ...
  assert.equal(getDocumentTextStrategy({ mime_type: 'text/rtf', name: 'report.rtf' }), 'direct');
  assert.equal(getDocumentTextStrategy({ mime_type: 'application/mbox', name: 'email.eml' }), 'direct');
  assert.equal(getDocumentTextStrategy({ mime_type: 'message/rfc822', name: 'forwarded.msg' }), 'direct');
...
test('document records fall back to extension-based detection for rtf and eml', () => {
  assert.equal(getDocumentTextStrategy({ mime_type: 'application/octet-stream', name: 'report.rtf' }), 'direct');
  assert.equal(getDocumentTextStrategy({ mime_type: 'application/octet-stream', name: 'email.eml' }), 'direct');
});
...
test('matter document search order falls back to created desc when updated dates are missing', () => {
  const sorted = sortMatterDocumentsForSearch([
    { internal_id: 50, date_update: null, date_create: '2024-01-01T00:00:00Z' },
    { internal_id: 40, date_update: null, date_create: '2024-01-03T00:00:00Z' },
    { internal_id: 30, date_update: null, date_create: '2024-01-02T00:00:00Z' },
  ], helpers);

  assert.deepEqual(sorted.map((record) => record.internal_id), [40, 30, 50]);
});
