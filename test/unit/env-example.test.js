...
test('.env.example exposes phase 2 OCR configuration', () => {
  ...
  assert.match(envExample, /^LETTER_WRITER_ENABLED=/m);
  assert.match(envExample, /^AWS_REGION=/m);
  assert.match(envExample, /^AWS_BUCKET_NAME=/m);
  assert.match(envExample, /^S3_PREFIX=/m);
  assert.match(envExample, /^PRESIGN_EXPIRES_SECONDS=/m);
  assert.match(envExample, /^LETTER_WRITER_MCP_SHARED_SECRET=/m);
  assert.match(envExample, /^LETTER_WRITER_MCP_SHARED_SECRET_HEADER=/m);
